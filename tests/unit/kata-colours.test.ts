...
const KATA_CSS_PATH = 'src/styles/base.css';
...
// second hardcoded list — so the palette cannot silently drift from the content
// it transcribes. The stylesheet is base.css, vendored from the design-system
// repository; this repository owns kata.json, so it is where a mismatch fails. See tests/unit/belt-colours.test.ts, which does the same
// for the belt palette.
describe('base.css carries every kata\'s exact colour from kata.json', () => {
